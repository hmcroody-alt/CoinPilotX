...
**In the live database but NOT created anywhere in code — 5 real names:**

`comm_v2_pinned_messages` (0 rows), `business_os_confirmation_grants`,
`seller_application_assignments`, `seller_application_notes`, `seller_application_status_history`.

> **⚠ MAJOR CORRECTION — this section previously claimed 23 db-only tables, including all ~20 `comm_v2_*` tables, and concluded that "a fresh deploy would not recreate" the live messaging engine. That was wrong.**
>
> Eighteen of the nineteen `comm_v2_*` tables are declared with explicit `CREATE TABLE IF NOT EXISTS` in **`pulse_communications_v2/models.py`** (`COMM_V2_TABLES`), executed by `ensure_schema()` at `:410`, which is called from `service.py:214`. Only `comm_v2_pinned_messages` (0 rows) genuinely lacks DDL.
>
> **Cause:** this database extraction pass never scanned the `pulse_communications_v2/` package. The API extraction pass did. A per-directory scan that silently skips one package produces a confident, precise, and entirely false architectural conclusion — and this one cited a row count, which made it *more* believable rather than less.
>
> **Corrected totals for §1.3:** **835** table names declared in code (not 813), **65** code-only (not 57), **5** db-only (not 23). Live table count is **775**; the 776 reported elsewhere counted `sqlite_sequence`, an SQLite internal. There are **19** `comm_v2_*` tables, not 20.

The `comm_v2_*` set is live: `comm_v2_conversations` holds 219 rows, `comm_v2_messages` 1,411, `comm_v2_participants` 458. It is a second-generation messaging engine that **is** reproducible from source — but only if you know to look in `pulse_communications_v2/`, since nothing in `bot.py` reveals it.
...
STACK B (comm_v2, live, DDL in pulse_communications_v2/models.py — 1,411 messages)
...
7. ~~**`comm_v2_*` (20 tables, 1,411 live messages) has no DDL in the repo.** A rebuild from source would not recreate the messaging engine that is currently in production use.~~ — **RETRACTED. FALSE.** 18 of 19 `comm_v2_*` tables are declared in `pulse_communications_v2/models.py` (`COMM_V2_TABLES`, `ensure_schema()` `:410`). Only `comm_v2_pinned_messages` (0 rows) lacks DDL. This extraction pass never scanned that package. See the correction in §1.3.
