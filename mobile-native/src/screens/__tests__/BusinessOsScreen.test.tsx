...
describe("Business OS hub", () => {
  ...
  it("opens the section landing page when a gated tile with one is tapped", async () => {
    ...
    for (const section of landing) {
      const view = await renderHub();
      fireEvent.press(view.getByLabelText(tileLabel(section)));
      // No `title` in the payload on purpose. The landing derives its header
      // from `businessOsSection(key).label`, so passing one would be a second
      // copy of the same string that a rename could silently desync. The route
      // type enforces this — `BusinessOsSection` takes `section` and nothing else.
