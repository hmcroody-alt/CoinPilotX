...
describe("Premium crypto routes resolve to the crypto screens", () => {
  ...
  it("leaves the rules the crypto family was moved above alone", () => {
    // The four rules jumped over verification, the account pages, the safety
    // hub, creator studio and growth. None of those paths contain a crypto
    // word, so the move should steal nothing — but `/alerts` was widened in the
    // same change, and a broad `includes` is exactly how a reorder turns into a
    // regression somewhere nobody was looking.
    expect(resolve("/dashboard/verification")[0]?.screen).toBe("VerificationCenter");
    expect(resolve("/dashboard/account/security")[0]?.screen).toBe("AccountCenter");
    expect(resolve("/dashboard/safety")[0]?.screen).toBe("SafetyHub");
