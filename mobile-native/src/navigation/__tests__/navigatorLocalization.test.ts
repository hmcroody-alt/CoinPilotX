...
describe("navigator header titles", () => {
  it("finds the title options it is meant to be checking", () => {
    ...
    // every assertion below vacuously pass. 107 stack screens (incl. the three
    // Page OS routes and the Presence hub) + 14 tabs, plus the `title` param
    // the header passes when opening the activity inbox.
    expect(TITLE_OPTIONS.length).toBe(124);
  });
