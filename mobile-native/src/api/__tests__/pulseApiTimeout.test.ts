...
describe("pulseApi request timeout", () => {
  ...
  it("gives a FormData upload a much larger budget than a JSON request", async () => {
    ...
    // Well past the JSON budget, an upload must still be running. This is the
    // property that stops a large video upload on a slow connection from being
    // cancelled by a budget meant for a JSON round trip.
