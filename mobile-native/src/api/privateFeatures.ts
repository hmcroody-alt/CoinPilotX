/**
 * Relationship Intelligence — the Private Office people surface — as one typed
 * client vocabulary.
 *
 ...
 *
 * This file used to carry five features. Documents, briefings, shield and the
 * concierge desk left the Private Office surface with their screens; their
 * readers are gone from here so nothing can quietly start calling them again,
 * and their endpoints are untouched on the server for clients still on an old
 * build. `privateFeatureRefusal` stays exported because
 * `privateConversations.ts` shares this refusal vocabulary.
 */
