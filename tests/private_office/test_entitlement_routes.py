...
# --- read-only-ness ---------------------------------------------------------
def test_pack_is_get_only():
    """Granting a tier belongs to the existing admin entitlement paths. A write
    registered here would be a second granting authority, so the absence of one
    is asserted over the URL map rather than left to code review."""
    app = _app()
    for rule in app.url_map.iter_rules():
        if not str(rule.rule).startswith(("/api/private-office",
                                          "/api/admin/private-office")):
            continue
        assert rule.methods <= {"GET", "HEAD", "OPTIONS"}, \
            f"{rule.rule} exposes {rule.methods}"
