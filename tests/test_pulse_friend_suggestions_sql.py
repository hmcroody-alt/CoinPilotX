...
def _seed(cur):
    cur.executescript(
        ...
        -- The suggested query joins `users` so it can apply the discovery
        -- predicate that keeps QA accounts out of People You May Know. Without
        -- this table the join raises, the surrounding `except` swallows it, and
        -- every assertion below would read an empty list as "nobody to suggest"
...
def _user(cur, user_id, *, hidden=0, status="active"):
    cur.execute(
        "INSERT OR REPLACE INTO users (user_id, hidden_from_discovery, account_status) VALUES (?, ?, ?)",
        (user_id, hidden, status),
    )


def _post(cur, user_id, created_at, deleted_at=None):
    # Authors are discoverable unless a test says otherwise, so every existing
    # case keeps expressing "a normal person posted" without restating it.
    _user(cur, user_id)
    cur.execute(
...
class SuggestedPeopleBehaviour(unittest.TestCase):
    ...
    def test_suggested_hides_qa_and_deactivated_accounts(self):
        """Pins both fixes at once, because they were made independently.

        The grouping keeps PostgreSQL from rejecting the query; the discovery
        predicate keeps QA accounts out of it. A later edit that reverts either
        one leaves the other looking healthy, so this asserts the combination.
        """
        _post(self.cur, 2, "2026-08-09T00:00:00")
        _post(self.cur, 3, "2026-08-08T00:00:00")
        _post(self.cur, 4, "2026-08-07T00:00:00")
        _user(self.cur, 3, hidden=1)
        _user(self.cur, 4, status="disabled_qa")
