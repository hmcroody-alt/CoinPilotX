...
def main() -> int:
    ...
    for source in (reel_player, status_player, viewer, live, calls, voice, radio):
        require("claimMediaPlayback" in source, "every active native media owner must use the shared coordinator")
    require("!active || !ownsPlayback || !attachedAudio" in reel_player, "offscreen or preempted Reels must not allocate attached-audio players")
    ...
    for route in ("/api/pulse/reels/feed", "/api/pulse/reels/create", "/api/pulse/reels/${reelId}/react", "/api/pulse/reels/${reelId}/comments", "/api/pulse/reels/${reelId}/save", "/api/pulse/reels/${reelId}/view"):
