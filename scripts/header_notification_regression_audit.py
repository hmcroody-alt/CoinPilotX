...
def main() -> int:
    ...
    require("pulse-desktop-nav" in desktop_top and "pulse-desktop-more-menu" in desktop_top, "desktop header missing restored destination nav or Apps menu", failures)
    require("data-header-notifications" in desktop_top, "desktop header missing Notifications marker", failures)
    require("pulse-alert-radar" in desktop_top, "desktop header missing restored radar-style Notifications control", failures)
    require("data-alert-unread" in desktop_top, "desktop header missing notification unread badge", failures)
    require('("Messages", "/pulse/messages")' in desktop_top, "desktop header missing restored Messages destination", failures)
    require("data-chat-unread" in desktop_top, "desktop Messages destination missing chat unread badge", failures)
    require("pulse-topnav-live" in desktop_top, "desktop header missing restored Live action", failures)
    require("pulse-create-strong" in desktop_top, "desktop header missing restored Create action", failures)
    require("pulse-topnav-messages" not in desktop_top, "desktop header reintroduced right-side duplicate Messages icon", failures)
