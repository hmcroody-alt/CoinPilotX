...
def main():
    ...
    require("max-width: min(25%, 220px)" in CSS and "linear-gradient(90deg" in CSS and "rgba(2, 6, 23, 0)" in CSS, "left metadata panel is a compact fading glass overlay")
    require("right: auto;" in CSS and "pointer-events: none;" in CSS and "overflow: hidden;" in CSS, "left metadata panel avoids hard edges and accidental media blocking")
    require('"reel-action", "reel-action-button"' in JS and "'reel-action','reel-action-button'" in BOT, "Status actions reuse the shared Reels action component")
    require(".pulse-status-story-actions .reel-action.reel-action-button" in CSS and "grid-template-rows: 26px auto" in CSS, "Status actions inherit shared Reels glass sizing")
    require("animation: reelActionPulse" in CSS and "@keyframes reelActionPulse" in CSS, "Status active states reuse the Reels pulse animation")
