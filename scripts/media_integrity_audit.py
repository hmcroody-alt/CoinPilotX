...
def main():
    ...
    expect(media_service._media_header_ok("jpg", b"\xff\xd8\xff\xe0"), "JPEG upload signature is accepted")
    expect(not media_service._media_header_ok("jpg", b"<script>alert(1)</script>"), "spoofed JPEG upload is rejected")
    expect(media_service._media_header_ok("mp4", b"\x00\x00\x00\x18ftypmp42\x00\x00\x00\x00mp42isom"), "MP4 upload signature is accepted")
    expect(not media_service._media_header_ok("mp4", b"not really a video"), "spoofed MP4 upload is rejected")
    expect(media_service._media_header_ok("webm", b"\x1a\x45\xdf\xa3\x9f\x42\x86\x81"), "WebM upload signature is accepted")
    expect(media_service._media_header_ok("mp3", b"ID3\x04\x00\x00\x00"), "MP3 upload signature is accepted")
    expect(media_service._media_header_ok("pdf", b"%PDF-1.7\n"), "PDF upload signature is accepted")
    expect(not media_service._media_header_ok("pdf", b"<html>not a pdf</html>"), "spoofed PDF upload is rejected")
    expect(media_service._media_header_ok("docx", b"PK\x03\x04\x14\x00"), "DOCX upload signature is accepted")
    expect(not media_service._media_header_ok("txt", b"hello\x00hidden"), "binary text upload spoof is rejected")
    conn = bot.db()
