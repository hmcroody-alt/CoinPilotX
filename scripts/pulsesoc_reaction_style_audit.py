...
def main() -> int:
    ...
    checks = [
        ...
        check("Status actions use shared class", '"pulse-status-action", "pulse-action-button"' in status_js),
        check("Inline status viewer uses shared class", "button.classList.add('pulse-status-action','pulse-action-button')" in bot_py),
        ...
        check("Status data actions exist", "button.dataset.action" in status_js and 'key === "love" ? "like" : key' in status_js),
