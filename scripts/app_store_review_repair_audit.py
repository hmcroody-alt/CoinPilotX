...
def main():
    ...
    require("bot.py", 'context.setdefault("paid_digital_access_available", paid_digital_access_available)', "account pages receive paid digital availability flag")
    require("bot.py", "if ios_native_app_request():\n        body = f\"\"\"", "iOS premium locked surfaces avoid paid upgrade CTA")
    require("templates/account.html", "{% if paid_digital_access_available %}\n            <a href=\"/pulse/premium\">PulseSoc Premium</a>", "account nav gates Premium on iOS")
    require("templates/account.html", "{% if not paid_digital_access_available %}", "iOS account fallback content")
    require("templates/account.html", "Paid digital access is not available in this iOS build.", "iOS account copy")
