...
def main() -> int:
    ...
    checks = [
        ...
        check("Media uses immersive cover behavior", "object-fit: cover !important" in feed_css and "height: clamp(360px, 67vw, 790px)" in feed_css),
        ...
        check("Reaction summary uses comments reposts shares saves only", all(token in section(home_js, "const metrics = [", "metrics.forEach") for token in ['"comments"', '"reposts"', '"shares"', '"saves"']) and '"views"' not in section(home_js, "const metrics = [", "metrics.forEach")),
        ...
        check("Reaction picker uses expanded backend reaction types", all(token in home_js for token in ['"like"', '"love"', '"funny"', '"wow"', '"brutal"', '"scam_alert"', '"fire"', '"fast_signal"', '"elite"', '"bullish"', '"bearish"'])),
        ...
        check("Comment composer compact V3 grid exists", ".feed > .pulse-feed-post-v3 .post-comment-composer" in feed_css and "grid-template-columns: 44px minmax(0, 1fr) 42px 42px 50px" in feed_css),
