...
def main():
    ...
    require("renderStatusMediaPreview" in source and "data-status-create-form='dedicated'" in source, "Status editor uses dedicated page JavaScript")
    require("pulse_status_rail_html" in source and "href='/pulse/status'" in source, "Pulse homepage routes Status creation to dedicated page")
    require("data-status2-form" in source and "Text" in source and "Photo" in source and "Video" in source, "Create Status opens the unified story chooser")
    ...
    require("data-status2-pick-media" in source and "statusMedia?.click()" in source, "Create Status directly opens media picker before preview")
    require("statusPlaceholderTypes" in source and "coming soon" in source.lower(), "unsupported status types show clear placeholder copy")
    ...
    require("data-open-status-id" in source and "/api/pulse/status/rail" in source, "Status page renders existing stories")
    require("statusGrid.innerHTML" in source and "data-open-status-id" in source, "Status cards render through the dedicated page")
    ...
    require(".pulse-status-home-entry" in css and ".pulse-status2-composer" in css and ".pulse-status2-preview" in css and ".pulse-status-full-grid" in css, "Status editor and viewer have immersive styling")
