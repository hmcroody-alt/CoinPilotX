...
def main():
    ...
    expect(bool(reg.get("reconnect_token")), "websocket reconnect token issued", str(reg))
    ...
    ack = websocket_orchestrator.acknowledge("audit-ws-1", event_id=1, sequence=1)
    expect(ack.get("ok") is True, "websocket delivery acknowledgement")
    rec = websocket_orchestrator.reconnect("audit-ws-1", reg.get("reconnect_token"), channel="pulse:conversation:audit")
    expect(rec.get("ok") is True, "websocket reconnect recovery")
    ...
    expect((health.get("recovery") or {}).get("delivery_ack") is True, "websocket recovery policy includes ack/replay", str(health))
