...
# Derived state
# ---------------------------------------------------------------------------
def allowed_transitions(record_type: str, status: object) -> tuple[str, ...] | None:
    """The statuses one legal move away from ``status``.

    ``None`` means the spec declares no matrix — the absence of a key must not
    silently mean "nothing is allowed". An empty tuple means the state is
    terminal. A status outside the vocabulary also gets the empty tuple: a
    record in a state the spec does not know has no sanctioned move except
    through :func:`revise_record`.
    """
    spec = _spec(record_type)
    matrix = spec.get("transitions")
    if matrix is None:
        return None
    return tuple(matrix.get(str(status or "").strip().upper(), ()))


#: Statuses in which a due date is live, per type. Derived OVERDUE / DUE_SOON
#: apply only while the record is genuinely awaiting action: a resolved
#: obligation with a past due date is resolved, not overdue, and a DRAFT or
#: DEFERRED task is one the member has explicitly said is not in play.
_DUE_DERIVABLE: dict[str, tuple[str, ...]] = {
    TYPE_OBLIGATION: ("OPEN",),
    TYPE_TASK: ("OPEN", "PLANNED", "IN_PROGRESS", "WAITING", "BLOCKED", "AWAITING_APPROVAL"),
    TYPE_PROJECT: ("OPEN", "PLANNED", "IN_PROGRESS", "WAITING", "BLOCKED", "AWAITING_APPROVAL"),
}
...
def effective_status(record_type: str, row: dict, *, now: datetime | None = None) -> str:
    ...
    Derived, never stored: a stored OVERDUE would need a sweeper to be true,
    and would be false between sweeps. Applies to the types that carry a
    ``due_at`` (obligations, tasks, projects), and only in their live states.
    """
    stored = str(row.get("status") or "")
    live = _DUE_DERIVABLE.get(record_type)
    if not live or stored not in live:
        return stored
...
def update_record(
    ...
    **fields,
    ...
    if "status" in fields:
        ...
        prior_status = str(dict(current).get("status") or "")
        if status != prior_status:
            # The matrix is enforced here, at the one writer, rather than in
            # the routes: a rule enforced where the write happens cannot be
            # bypassed by the next caller. Same-status writes pass untouched —
            # a client retry is not a transition.
            legal = allowed_transitions(kind, prior_status)
            if legal is not None and status not in legal:
                raise PrivateRecordRejected(
                    f"cannot move {kind} from {prior_status} to {status}; "
