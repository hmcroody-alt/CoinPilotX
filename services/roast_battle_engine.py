...
def ensure_participant(match_id, user_id):
    ...
    if not existing:
        cur.execute(
            ...
            """,
            (int(match_id), int(user_id), f"PulseSoc-{int(user_id)}", call_sign, DEFAULT_STAGE_BALANCE, DEFAULT_STAGE_BALANCE, now),
        )
    else:
        cur.execute(
...
def submit_message(user_id, match_id, message, target_user_id=None, target_type="single", target_public_player_id=None):
    ...
    sender_public_id = (sender_profile["public_player_id"] if sender_profile else "") or f"PulseSoc-{int(user_id)}"
