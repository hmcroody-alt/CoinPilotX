"""World presence feed that makes CoinPlotXAI feel alive without fake users."""
...
def world_feed(limit=12):
    ...
    items.append({"type": "trust", "title": "Scam Shield lesson", "body": "Verify before signing. CoinPlotXAI never asks for seed phrases.", "url": "/scam-shield"})
    live_events = live_event_engine.poll("global", after_id=0, limit=5)
    for event in live_events[-5:]:
        payload = event.get("payload") or {}
        items.append({"type": event.get("event_type"), "title": payload.get("title") or "Live platform event", "body": payload.get("body") or "CoinPlotXAI live event", "url": payload.get("url") or "/arena/play"})
