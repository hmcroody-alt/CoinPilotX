...
FOUNDATION: tuple[Responsibility, ...] = (
    ...
    Responsibility(
        ...
        gap=(
            ...
            "by this sentence\". The canonical runtime now opens the workspace, places "
            "attention's bounded focus into it, and refuses selection outside that "
            "focus. All three flags still default off for rollback."
        ),
        note=(
    ...
    Responsibility(
        ...
        gap=(
            ...
            "The canonical runtime now calls ``execute`` and hands it the existing "
            "governed gateway as performer. With ``UNDX_BRAIN_EXECUTOR_ENABLED`` "
            "defaulting off the legacy path "
            "remains; when enabled, calls and retries spend the Brain ledger. Plan "
            "execution in the runtime is still one step per request, so true multi-step "
    ...
    Responsibility(
        ...
        gap=(
            ...
            "``undx_agent_runtime`` now calls ``select`` before the legacy matcher. A "
            "decided result becomes the proposal and an unresolved consequential band "
            "becomes a clarification. ``UNDX_BRAIN_SELECTION_ENABLED`` defaults off, "
            "preserving the legacy "
            "single-answer matcher as rollback. "
    ...
    Responsibility(
        ...
        gap=(
            ...
            "one proposed call and stops. ``undx_agent_runtime`` now calls ``predict`` "
            "after argument resolution and before the gateway, then calls ``check`` "
            "against the independent read-back. A mismatch removes completion permission "
            "from the card. ``selection`` also imports prediction for contested writes. "
            "With "
            "``UNDX_BRAIN_PREDICTION_ENABLED`` defaults off; both entry points answer ``ok=False``, "
    ...
    Responsibility(
        ...
        gap=(
            "Ten analysers, against a capability registry that grows with every feature "
            "shipped anywhere in the product. Account health, verification, support "
            "tickets, groups, events, music, creator analytics, presence privacy and "
            "localisation have a specialist reading; every other capability — "
            "Marketplace, crypto alerts, feed posts, reels, messages, saved content, "
            "security, premium, ads, live — falls back to the shape-only layer, which "
            "counts records and reports degradation and says nothing about what the "
            "records mean. That fallback is correct behaviour and it is also the gap: "
            "most capabilities answer with arithmetic.\n\n"
            "The registry and product-area totals are deliberately not written down "
            "here, and putting them back would undo a fix. They move whenever anyone "
            "ships anything, so a numeral in the paragraph above goes stale without "
            "specialist coverage having changed at all — which is exactly what happened "
            "to the three totals it used to carry, all of them wrong within six weeks "
            "and two of them wrong without anybody noticing. The count that belongs to "
            "this entry is the analyser count, because that one moves only when "
            "somebody writes an analyser and edits this text in the same change; it is "
            "stated once, here, and the test reads it back out rather than keeping a "
    ...
    Responsibility(
        ...
        gap=(
            ...
            "row."
            "\n\n"
            "``undx_agent_runtime`` now reads owner-scoped calibration before dispatch. "
            "When the minimum sample is conclusive and corrections outnumber approvals, "
            "it may only add a clarification before a write; it never down-ranks a "
            "capability or selects an alternative. ``UNDX_BRAIN_CALIBRATION_ENABLED`` "
            "still defaults off. The third question — whether one "
            ...
            "the reader can infer around. Noticing is still not learning: it may narrow "
            "a write into clarification, but it never expands authority or rewrites the "
            "selection score."
        ),
        note=(
    ...
    Responsibility(
        ...
        gap=(
            ...
            "called directly from the live path.\n\n"
            ...
            "Reach, which is the part that moved and the part that did not. Two modules "
            "call ``facts`` — ``services.undx_brain.learning`` uses ``facts.read`` to "
            ...
            "rate it cannot date. This entry used to add that neither of those callers "
            "was itself reachable from a request. That is no longer true: "
            "``undx_agent_runtime.handle`` calls ``learning.load`` and "
            "``calibration.calibrate`` on every recognised action, so ``facts`` now sits "
            "two hops from a real turn rather than in a closed loop of readers. Two hops "
            "is not one, and the distinction is the honest one to draw — no runtime code "
            "asks ``facts`` a question directly, and ``record_fact``'s writes are still "
            "read by nothing on the live path — but \"unreached\" was the wrong word and "
            "is withdrawn. ``facts`` is behind "
    ...
    Responsibility(
        ...
        gap=(
            ...
            "Reach, which is narrower than it was and narrower than it sounds. This "
            "entry used to say that nothing calls ``learning``. That is no longer true: "
            "``undx_agent_runtime.handle`` opens an owner scope and calls "
            "``learning.load`` on every turn that resolves to an action, and hands the "
            "window it returns to ``calibration.calibrate``. So the reader is reached "
            "from a real request. What that reach *buys* is still almost nothing, and "
            "saying otherwise would be the more flattering error: the module sits behind "
            "``UNDX_BRAIN_LEARNING_ENABLED``, which defaults off, and with it off "
            "``load`` answers ``ok=False`` so that a disabled reader is never mistaken "
            "for one that looked and found nothing. The one behaviour the live path "
            "derives from the window, once the flag is on, is calibration's single "
            "clarification before a write — nothing reads ``distribution`` or "
            "``succession`` outside their own tests. The admin dashboard still "
            "counts.\n\n"
    ...
    Responsibility(
        ...
        gap=(
            ...
            "Runtime regulation now has a narrow first owner: prediction unavailability "
            "fails a Brain-enabled write closed, execution-ledger refusal blocks dispatch, "
            "and degraded reads are marked for disclosure. Broader adaptation remains "
            "absent: nothing measures current load, latency, provider health or refusal "
            "rate, and nothing widens a limit in response. There is no signal a "
