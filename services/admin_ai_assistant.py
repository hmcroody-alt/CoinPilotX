...
def summarize_incidents(metrics=None):
    ...
    if int(metrics.get("flagged_posts") or 0):
        issues.append("Trust & Safety has flagged Pulse content waiting.")
    if int(metrics.get("failed_messages") or 0):
        issues.append("Pulse Messenger delivery should be inspected.")
    return {"summary": " ".join(issues) if issues else "No urgent Pulse incidents detected.", "issues": issues}
