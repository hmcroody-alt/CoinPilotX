...
def trade_commentary(player_name: str, symbol: str, side: str, quantity: float, price: float, psychology: dict):
    ...
    if discipline >= 80:
        return f"{player} placed a controlled fake {side} on {symbol}. Discipline score improved while keeping risk visible."
    if fomo >= 55:
        return f"{player} entered {symbol} with elevated FOMO pressure. AI recommends smaller sizing and confirmation before the next fake trade."
    return f"{player} executed a fake {side} on {symbol} worth about ${notional:,.2f}. {arena_psychology.match_psychology_summary(psychology)}"
...
def match_recap(match: dict, participants: list, events: list):
    ...
    if leader:
        return (
            ...
            f"with {int(leader.get('score') or 0)} points. Risk-adjusted discipline matters more than fake profit."
        )
    if events:
        return "The match is active. AI is watching fake trades, discipline, and scam-defense decisions."
...
def room_commentary(room_title: str, active_matches: int, top_player: str = ""):
    if active_matches:
        return f"{room_title} is live with {active_matches} active battle updates. Watch discipline and risk control, not just fake P/L."
    if top_player:
