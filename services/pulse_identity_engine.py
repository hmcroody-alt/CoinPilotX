"""Central PulseSoc social identity helpers.

Normal PulseSoc surfaces use display name, username, avatar, social labels, and
...
def primary_label(row=None, badge_keys=None, badge_labels=None) -> str:
    ...
    if premium_identity_engine.is_owner(row) or {"owner", "founder", "founder_badge"} & badge_key_set:
        return "Founder · PulseSoc"
...
def build_identity(row=None, badge_keys=None, badge_labels=None) -> dict:
    ...
    name = (
        ...
        or row.get("username")
        or f"PulseSoc Member #{str(public_id)[-4:]}"
    )
    ...
    return {
        ...
        "is_owner": bool(premium_identity_engine.is_owner(row) or {"owner", "founder", "founder_badge"} & badge_set),
