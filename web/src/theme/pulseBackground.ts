/**
 * The PulseSoc backdrop tokens, mirrored from the native app.
 *
 * Source of truth: `mobile-native/src/theme/pulseBackground.ts`, plus
 * `galacticProfileFor` in `mobile-native/src/theme/ThemeContext.tsx`. Every
 * number here is carried verbatim rather than re-derived, and
 * `scripts/ops/native_background_parity_gate.py` compares the two files value
 * by value. Adjusting a coordinate here without adjusting it there is a gate
 * failure, which is the intended way to find out.
 *
 * The two rules native encodes are encoded here for the same reasons:
 *
 *  1. Opacity ceilings. `nodeOpacity`/`lineOpacity` clamp, so no node is ever
 *     painted above 0.20 and no line above 0.12 regardless of what a variant
 *     asks for. Text sits above this layer and has to stay readable on the
 *     worst screen at the worst brightness. They are functions and not
 *     constants precisely so the clamp cannot be skipped at a call site.
 *
 *  2. Determinism. Node and line positions are a fixed table, never random. A
 *     random field cannot be reviewed, cannot be regression-tested, and
 *     re-rolls on every remount -- which reads as the backdrop twitching.
 *
 * Light themes get their own surface rather than a dimmed dark one: painting a
 * near-black navy field under the Light Futuristic palette is a contrast
 * inversion, not a subtler backdrop.
 */
...
export const PULSE_BACKGROUND_CEILINGS = {
   ...
   * layer's own opacity is applied. It is a lift, not a wash.
   */
  bottomGlow: 0.1,
} as const;
...
/**
 * Geometry shared by every variant. Sizes are CSS px, which is what native's
 * "device-independent px" means on this platform.
 */
...
export const PULSE_BACKGROUND_SURFACES: Record<"dark" | "light", PulseBackgroundSurface> = {
   ...
   * nodes at roughly half strength.
   */
  light: {
    gradient: {
...
/**
 * Which backdrop treatment a theme gets, mirrored from `galacticProfileFor` in
 * native's `ThemeContext.tsx`.
 *
 * `enabled: false` for White is not a styling preference, it is the reason the
 * component has an early return: White renders no backdrop at all. `intensity`
 * is the whole field's opacity, which is how Black gets the same composition at
 * a little over half strength rather than a separate dimmer table.
 */
...
export function galacticProfileFor(
  mode: string,
  scheme: "light" | "dark",
): GalacticBackgroundProfile {
...
/**
 * A CSS `linear-gradient()` built from a stop table.
 *
 * Native's `LinearGradient` takes colours and locations as parallel arrays and
 * paints top-to-bottom by default; `180deg` is the CSS spelling of the same
 * direction. Building the string from the table rather than writing it out
 * keeps the table the only place a stop can be changed -- a hand-written
 * gradient in a stylesheet would be a second copy that the parity gate, which
 * reads this file, could not see.
 */
export function gradientCss(stops: GradientStops): string {
  const parts = stops.colors.map(
    (color, index) => `${color} ${(stops.locations[index] ?? 0) * 100}%`,
  );
