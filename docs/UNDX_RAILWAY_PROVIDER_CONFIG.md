...
| `CLAUDE_AI_API` | PRESENT | Claude | Set, but **the live check fails 404** — see "Known-broken". |
...
| `CLAUDE_MODEL` | not set | `claude-3-5-haiku-latest` |
...
### Claude — 404 from `api.anthropic.com`

`CLAUDE_AI_API` is set, and `ANTHROPIC_BASE_URL` is also set in this environment.
The 404 is consistent with the credential belonging to a gateway rather than to
Anthropic's public API, while `undx_router._call_claude` posts to
`https://api.anthropic.com/v1/messages` unconditionally. Not investigated
further here. Claude is a named specialist in the routing chains and is
currently **dead in production**.
