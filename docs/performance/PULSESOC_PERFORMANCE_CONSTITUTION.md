...
from a hang. Ordinary JSON requests are bounded at `PULSE_API_READ_TIMEOUT_MS`
(15s); `FormData` uploads at 180s, because cancelling a legitimate video upload
on a slow connection is a data-loss bug wearing a performance costume.

The budget covers **every** method, and applies **even when the caller supplies a
`signal`**. Both halves matter, and an earlier read-only deadline had neither: it
bounded GETs only, so every write could hang forever, and it opted out entirely
when a `signal` was present — leaving precisely the requests that already had
cancellation wiring as the unbounded ones.
...
timeout (`504` / `request_timeout`) — a caller cancellation is not (`503` /
`request_unreachable`), and must not be described to the user as the server being
slow.

*Enforced by:* `src/api/__tests__/pulseApiTimeout.test.ts` (5 tests) and
`pulseApiCoalescing.test.ts` — "rejects a stalled canonical GET instead of
leaving its screen loading forever".
