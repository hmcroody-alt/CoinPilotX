...
### How to apply it, and what that costs

There is no `expo-build-properties` in `mobile-native/package.json`, so the
value lives only in the committed pbxproj. Two edits are needed and both should
land together, because either alone drifts:

1. The four `IPHONEOS_DEPLOYMENT_TARGET` lines in the pbxproj.
2. `expo-build-properties` with `ios.deploymentTarget: "16.1"` in `app.json`,
   so a non-clean prebuild does not quietly put 15.1 back.

Step 2 edits `mobile-native/app.json`, which is a `dependency_watch` trapped
path (`config/realtime-audio-protected-paths.json:507`). That means the full
battery plus a fresh `reports/realtime_audio_change_declaration.md`. This is a
real cost and it is listed here rather than discovered later. It is not a reason
to skip step 2 — skipping it is how the floor silently reverts.
