...
| 9 | MapKit | — | `MAPKIT.md`. **Not recommended — and not because the data is merely missing.** `visitor_sessions.latitude/longitude` are dead columns bound to literal `NULL` with no reader; region and city were deliberately emptied because the only source was client-asserted (`bot.py:15058-15063`, "record nothing rather than recording a claim"). The one structured, geocodable address data in the schema is `admin_users` / `employees` — **staff home addresses**, which Finding 4 makes a named prohibition. MapKit itself is free (SwiftUI `Map` is iOS 14.0, no entitlement); *location* is not — `NSPrivacyCollectedDataTypes` is currently empty and the web sets `geolocation=()` |
