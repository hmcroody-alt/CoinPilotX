...
| issue | detail |
|---|---|
| **IP keying behind shared egress** | `client_ip_hash()` (`bot.py:14449`) took the **first** element of `X-Forwarded-For` — the client-supplied leftmost hop — until 2026-09-13. It now resolves through `services/client_address.py`, which counts hops from the right; a probe confirmed Railway's edge *replaces* the header, so the fix is a no-op in production today and exists to keep it one. The NAT collapse is untouched by that: an office/school/mobile-carrier NAT still puts hundreds of browser users in one bucket for the two IP-only limiters (§6.1, §6.3) — including `/login` at 12/300s. See `PULSESOC_WEB_SECURITY_MODEL.md` §6.1–§6.3. |
