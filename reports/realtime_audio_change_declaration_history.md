# Real-Time Audio Change Declarations — Archive

Superseded declarations, kept in full rather than left to `git log`. A declaration is
the only written record of what a protected change claimed about itself, so the version
that shipped should stay readable without archaeology.

The declaration for the change currently under review lives in
...
**Status:** SHIPPED — commit `b8d4dd43`, snapshot tag `realtime-audio-stable-v1`
