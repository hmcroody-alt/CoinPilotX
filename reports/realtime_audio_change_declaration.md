...
## Final consolidation addendum (2026-09-17)

This discharges the "Still owed" section above and declares one further
protected file, on branch `release/final-consolidation-20260917` integrated onto
`origin/main` `321f1c1c4e16a8ddf185bed67d9b9b676cc5b574`.

### Why the change is required

The translation module was declared in `mobile-native/package.json` and shipped
a podspec, but `mobile-native/ios/Podfile.lock` had never been regenerated. Expo
autolinking therefore had nothing to link, `requireOptionalNativeModule`
returned nil on device, and every call site correctly degraded to the typed
`native_bridge_unavailable` failure with cloud fallback permitted. The app was
not broken by this — it was quietly running the cloud path in a build that
claimed an on-device one. `pod install` was run to close that gap.

### Which feature required it

Apple on-device translation only. No Agora, Mux, LiveKit, AVAudioSession,
microphone-publication or livestream code was touched, and no protected
*category* path (as opposed to `dependency_watch`) changed in this range.

### Which protected files changed

| File | Category | Change |
|---|---|---|
| `mobile-native/ios/Podfile.lock` | dependency lock | Regenerated by `pod install`. Adds `PulseAppleTranslation (1.0.0)` from `../modules/pulse-apple-translation/ios` plus its checksum and dependency entries — six added lines, none removed. No existing pod added, removed, re-pinned or version-changed. `react-native-agora` and `PulseNowPlaying` are unchanged and still linked. |
| `mobile-native/package.json` | dependency watch | One line, carried in from the feature branch: `"pulse-apple-translation": "file:./modules/pulse-apple-translation"`. A local `file:` module, not a third-party package. No version changed on any existing dependency. |
| `mobile-native/package-lock.json` | dependency lock | The `pulse-apple-translation` link entries only. |

### Validation run on this range

Every item the gate requires, run against this branch:

| Check | Result |
|---|---|
| `npm run typecheck` (`tsc --noEmit`) | clean, exit 0 |
| `npm test` (full Jest) | 467 suites, 8055 tests, all passed |
| `npm run test:realtime-audio-critical` | 11 suites, 191 tests, all passed |
| `npm run test:realtime-audio` | 21 suites, 377 tests, all passed, exit 0 |
| `npm run test:realtime-audio-architecture` | 1 suite, 22 tests, all passed |
| `tests/protection/test_realtime_audio_architecture.py` | 19 tests, OK |
| `test_agora_token_generation.py` + `test_agora_rtc_provider_contract.py` | 13 passed |
| `scripts/protection/run_protection_suite.py` | 687 checks across 46 suites, all passed |
| `scripts/test_apple_translation_swift.sh` | 57 tests, 194 assertions, PASS |
| `scripts/typecheck_apple_translation_swift.sh` | OK against the real iOS SDK and the real prebuilt `ExpoModulesCore.swiftmodule` |
| `npm run i18n:validate` | 11 locales at 100% (4612/4612), exit 0 |
| Native build verification | device and simulator builds from this SHA — see `docs/release/FINAL_DEVICE_INSTALL_REPORT.md` |

Physical audible validation is recorded in
