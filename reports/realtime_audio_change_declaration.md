...
## Consolidation addendum 5 — the guard's file-list mode was blind to bot.py (2026-09-13)

This addendum declares the protected change in `1cdab480`, carried onto `main` by
the merge `495cddc2` during the multi-agent consolidation of 2026-09-13.

### Why the change is required

`bot.py` is the one protected surface gated on diff **content** rather than on
path: only lines matching `backend_diff_patterns` count as an audio change, so
the guard must read a diff to answer the question at all. `--changed-files-from`
supplies a list of filenames and no commit range, so the backend half of the check
had nothing to read and was skipped outright on every file-list invocation.

The consequence is the failure mode this repository cares about most, because it
is the silent one: the skip was reported as an all-clear. File-list mode is
precisely the mode a person reaches for when scoping the guard to their own edits
in a dirty shared checkout — the documented workaround for the dependency-watch
trap — so the most-edited protected surface in the repo returned a guaranteed
pass in the exact circumstance it was most likely to be consulted. Nothing went
red. Nothing was logged. The guard simply stopped asking.

### Which feature required it

No product feature. This is guard repair — the `audio_governance` category,
which exists so that edits to the guard itself cannot be made quietly. No audio
feature was requested, designed, or shipped alongside it.

### Which protected files changed

| File | Category | Change |
|---|---|---|
| `scripts/realtime_audio_change_gate.py` | `audio_governance` | File-list mode now diffs `bot.py` against the working tree (unstaged and staged both) instead of skipping the backend check. An empty diff is treated as **unverifiable** and exits 2 rather than passing, because "git could not answer" and "nothing changed" are indistinguishable there. The line-scanning half of `backend_diff_is_audio_related` is split out as `patterns_in_diff` so both callers share one matcher; the public signature is unchanged. |

No other protected path changed in this range. `bot.py` itself changed
substantially in this consolidation (admin page HTML escaping, `495cddc2`..`18fcac3a`),
and the guard was run against the full range specifically to test that: it
reports no audio-related content in that diff, which is the answer the escaping
work should produce and now the answer the guard is actually capable of giving.

### The audio-relevant statements

None. The diff contains no `AVAudioSession`, no `setAudioModeAsync`, no track,
publication, route, engine, or session line. It changes only how the guard reads
a diff. Every real-time audio invariant in the product is byte-identical before
and after.

### Expected behavior change

For the product: none. This file is not imported by the app, the backend, or any
worker; it runs in CI and from a developer shell.

For the guard: `--changed-files-from` gains a third outcome. It previously
answered "clean" or "protected paths changed". It can now also answer "could not
verify" with exit 2, which is the correct report when the backend content check
cannot be performed. Any CI step treating non-zero as failure keeps working; a
step that only tested `== 1` would now let an exit 2 through, and there is none.

### Regression risk

Low, and asymmetric in the safe direction: the change can only make the guard
notice more, never less. The one real risk is exit-2 noise — a working tree where
`git diff bot.py` legitimately returns empty now halts instead of passing. That
is deliberate. A guard that cannot check must not report success; the whole point
of this repair is that the previous behaviour did exactly that.

### Tests run

Against this exact tree:

- `tests/protection/test_realtime_audio_gate_file_list_mode.py` — **13 tests, OK.**
  This suite is the direct regression lock on the repaired mode.
- `tests/protection/test_realtime_audio_gate_coverage.py` — **14 tests, OK.**
- `tests/protection/test_realtime_audio_architecture.py` — **19 tests, OK.**
- `scripts/realtime_audio_change_gate.py --base origin/main --head HEAD` — run
  against the whole consolidation range; correctly identifies this file as the
  only changed protected path and finds no audio content in the `bot.py` diff.
- Full backend protection suite — see the consolidation commit; green on a tree
  pinned for the run rather than on the shared checkout, which mutated mid-run
  and produced four spurious failures before the cause (`05bb07c4`) was found.

### Physical validation required

**Not required, and none is claimed.** The protected diff contains no session,
track, publication, route, or engine line — it is CI tooling. No audible
validation can be performed on a change that does not reach a device, and
asserting one here would be the kind of unearned claim this declaration format
exists to prevent. Ordinary release QA on device (audio call, video call,
livestream) is carried by the release that ships this consolidation.

### Rollback procedure

Revert `1cdab480`. `scripts/realtime_audio_change_gate.py` is self-contained and
imported by nothing at runtime, so reverting it restores the previous guard
