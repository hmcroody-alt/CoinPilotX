...
Audited public-facing app text, page titles, metadata, manifests, Roast Battle, Pulse Live defaults, post titles, and visible Pulse UI fallbacks for old `CoinPlotXAI Pulse` style branding.
...
- `PulseSoc™ • Built by CoinPlotXAI Inc.` remains valid footer/legal support text.

### Public-facing UI changed

- Roast Battle browser title and preview title now use `Pulse Roast Battle`.
- Roast Battle Pulse shell header now shows `Pulse`, not `CoinPlotXAI Pulse`.
...
- Admin, UNDX, Telegram bot, and legacy infrastructure references to `CoinPilotX`/`CoinPlotXAI` were not blindly replaced because they are operational or legal surfaces.
