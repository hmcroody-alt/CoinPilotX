...
1. **Frozen commit:** Working tree has **238 uncommitted changes** (112 modified, 126 untracked) including real source in `services/`, `mobile-native/`, and core `*.py`. The audited state is the working tree, **not a frozen commit**. The shipped build must be built from a committed SHA that matches what was audited.
