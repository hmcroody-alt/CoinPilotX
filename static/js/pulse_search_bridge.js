(function () {
  ...
  function resultHtml(item, query) {
    ...
    return `<a class="pulse-search-result" href="${escapeHtml(appFirstHref(item))}"><span class="pulse-search-mark">${escapeHtml(letter)}</span><span><strong>${highlight(item.title || "PulseSoc result", query)}</strong><small>${highlight(item.description || item.meta || "", query)}</small></span><span class="pulse-search-type">${escapeHtml(item.type || "PulseSoc")}</span></a>`;
  }
